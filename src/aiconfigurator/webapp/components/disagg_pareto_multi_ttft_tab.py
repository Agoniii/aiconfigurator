...
def create_disagg_pareto_multi_ttft_tab(app_config):
    with gr.Tab("Disaggregation Pareto Multi-TTFT Estimation"):
            ...
            introduction = gr.Markdown(
                ...
                value=r'''
                    **Please read the readme tab before using this tab.**  
                    This mode is used to estimate **multiple Pareto frontiers** for different TTFT constraints.  
                    ...
                    **Multi-TTFT feature**: Input multiple TTFT values (comma-separated) to compare different latency requirements on the same plot.  
                    Example: TTFT = "100,200,300" will generate three Pareto curves for comparison.  
        ...
        estimate_btn = gr.Button("Estimate Disaggregation Multi-TTFT Pareto")
        ...
        result_df = gr.Dataframe(
            label="Disaggregation multi-TTFT pareto datapoints (combined)",
            headers=['index']+ColumnsDisagg+['ttft_constraint'],
            interactive=False,
            visible=True
        )
